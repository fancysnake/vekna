...
Bare `vekna` (no subcommand) prints help listing the available subgroups;
a default behaviour will be added in a later release.
...
| Command | Effect |
|---------|--------|
| `vekna` | Print help listing available subgroups; reserved for a future default behaviour |
...
- One daemon owns `/tmp/vekna-<uid>.sock` and manages all sessions. `vekna
  tmux` starts it automatically on first use via `os.fork()`.
