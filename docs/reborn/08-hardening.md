...
**Version:** `1.0.0` — **in progress**; the publishing half ships at `0.5.0`.

> **Split.** Publishing to PyPI, the documentation inventory below, the example
> library and the `deptry` / `pip-audit` sweep come forward to `0.5.0` with
...
0.8.0) is ready. 1.0 is the readiness line, not a new capability.
