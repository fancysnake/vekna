...
Work is filed under the name from the day it starts; the number is attached at
the tag.
...
- One name per **major** release. The `0.x` line is the road to Reborn, not a
  series of names.
- The name is documentation, not packaging. Versions stay semver; the name
  rides in the changelog heading (`## 1.0.0 — Reborn`) and the track's
  directory.
- Name a track when work on it starts, not when it ships.
...
is what happened, this is what is meant to. Each feature doc repeats its own
status in its header and nowhere else.
